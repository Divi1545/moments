...
function appendMessage(message, animate = false) {
    ...
    <div class="message-content">
      ...
      ${!isOwn ? `
        <div class="message-actions">
          ...
          <button class="message-block-btn" data-user-id="${message.user_id}" data-user-name="${displayName}">Block User</button>
        </div>
      ` : ''}
    </div>
  `;
  ...
  if (!isOwn) {
    ...
    const blockBtn = messageEl.querySelector('.message-block-btn');
    blockBtn.addEventListener('click', () => {
      userToBlock = {
        id: message.user_id,
        name: displayName
      };
      document.getElementById('blockUserName').textContent = displayName;
...
function setupEventListeners() {
  ...
  document.getElementById('confirmBlockBtn').addEventListener('click', async () => {
    ...
    if (error) {
      if (error.code === '23505') {
        showToast('User already blocked', 'error');
      } else {
        console.error('Block error:', error);
        showToast('Error blocking user: ' + error.message, 'error');
      }
    } else {
      showToast(`${userToBlock.name} has been blocked`, 'success');
      // Refresh the page to hide blocked user's messages
      setTimeout(() => {
